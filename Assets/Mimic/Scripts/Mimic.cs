...
namespace MimicSpace
{
    public class Mimic : MonoBehaviour
    {
        ...
        public MeshRenderer mimicRenderer;
        ...
        void Start()
        {
            mimicRenderer=GetComponentInChildren<MeshRenderer>();
            ResetMimic();
        }
        ...
        void Update()
        {
            ...
            if (legCount <= maxLegs - partsPerLeg)
            {
                ...
                RaycastHit hit;
                Physics.Raycast(newLegPosition + Vector3.up * 10f, -Vector3.up, out hit,Mathf.Infinity,7);
                Vector3 myHit = hit.point;
                //Debug.Log(hit.transform.gameObject.layer.ToString());
                if (Physics.Linecast(transform.position, hit.point, out hit,7))
                    //Debug.Log(hit.transform.gameObject.layer.ToString());
                    myHit = hit.point;
        ...
        public void LegAttack(Vector3 attackPosition)
        {
            float lifeTime = Random.Range(minLegLifetime, maxLegLifetime);
            RequestLeg(attackPosition, legResolution, maxLegDistance, Random.Range(minGrowCoef, maxGrowCoef), this, lifeTime);
